...
#these specified columns will be the headers within my csv file in excel formatting
csv_columns = ['name','alternateName','Species','Gender','House','DateofBirth','YearofBirth','Wizard','Ancestry','EyeColour','HairColour', ]

#wrapped my dictionary values within a variable 
dict_data = [
  {
    "name": "Harry Potter",
    "alternate_names": [],
...
#Name of the csv file that will automatically be created once the script is ran
csv_file = "Hogwarts.csv"

#This csv module contains DictWriter in which requires the name of the csv file it will write to and a list of object containing field names
#write header writes the first line within the csv file as it's field name
#the for loop that follows will then write each row in csv formatting
#data is specified as my dictionary values above
try:
...
#TroubleShooting/Errors 
        #the API data that was pulled from a forked GitHub Repo contained 'true' and 'false' values that we're missing quotations, thus outputting a name error stating 'true' is not defined
        #I initially attempted to include the URL of the Harry Potter Character List API, import requests and then run a request to get the data from the API URL to form the CSV file by each header specified
        #I kept receiving a 'TypeError string indices must be integers, not str' which does not align with my API data type which is text